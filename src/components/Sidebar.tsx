...
export default function Sidebar({ 
  ...
  const animateState = isMobile 
    ...
    : (isCollapsed ? "desktopCollapsed" : "desktopExpanded");
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="z-50 bg-zinc-900 border-r border-white/5 flex flex-col fixed inset-y-0 left-0 h-screen shadow-2xl md:shadow-none"
      >
        {/* Logo Section */}
        <div className="h-20 flex items-center px-6 border-b border-white/5 overflow-hidden gap-3 justify-between">
          ...
          {isMobile && (
            <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-white/50 hover:text-white md:hidden">
              <X className="w-5 h-5" />
            </button>
          )}
        </div>
        ...
        <nav className="flex-1 px-3 py-6 space-y-1 overflow-y-auto custom-scrollbar">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center p-3.5 transition-all relative group gap-4
                ${activeTab === item.id 
                  ? 'text-cyan-100' 
        ...
        {!isMobile && (
          <button
            ...
            className="absolute -right-4 top-1/2 -translate-y-1/2 w-8 h-8 bg-zinc-800 border border-white/5 rounded-full items-center justify-center text-white/50 hover:text-white hover:border-cyan-100/50 transition-all z-50 flex"
          >
